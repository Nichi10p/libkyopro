...
namespace nichi10p {
  ...
  class unionfind_with_potential {
    private:
      ...
      std::pair<int, potential_t> _leader(int);  // {leader, potential}
    public:
      /* constructor */ explicit unionfind_with_potential(int const &);
      int               leader(int);
      potential_t       potential(int);
      void              merge(int, int, potential_t);
      bool              same(int const &, int const &);
      potential_t       diff(int const &, int const &);
      int               size(int const &);
...
namespace nichi10p {
  ...
  template<typename potential_t>
  std::pair<int, potential_t> unionfind_with_potential<potential_t>::_leader(int x) {
    potential_t p{};
    // path halving
  ...
  template<typename potential_t>
  int unionfind_with_potential<potential_t>::leader(int x) {
    return _leader(x).first;
  }

  template<typename potential_t>
  potential_t unionfind_with_potential<potential_t>::potential(int x) {
  ...
  template<typename potential_t>
  bool unionfind_with_potential<potential_t>::same(int const &x, int const &y) {
    return leader(x) == leader(y);
  }

  template<typename potential_t>
  potential_t unionfind_with_potential<potential_t>::diff(int const &x, int const &y) {
    return std::minus{}(potential(y), potential(x));
  }

  template<typename potential_t>
  int unionfind_with_potential<potential_t>::size(int const &x) {
    return -par[leader(x)];
  }
}
